# Installing Spacecraft Software Theme for Google Chrome
...
`themes/<slug>/manifest.json`. Chrome installs exactly one theme per
extension, so each theme is its own loadable folder and its own zip —
there is no in-browser theme picker the way an editor has one.
...
3. Drag and drop `themes/spacecraft-software-chrome-<slug>.zip` onto the page.

## Method 3: Chrome Web Store

1. Package a `themes/<slug>/` folder as a `.crx` using Chrome's extension tools.
