...
`themes/<slug>/manifest.json`. Trivalent is Chromium-based and installs
exactly one theme per extension, so each theme is its own loadable folder
and its own zip.
...
3. Drag and drop `themes/spacecraft-software-trivalent-<slug>.zip` onto the page.

## Notes

- This theme changes only the visual appearance — it does not affect
  Trivalent's security hardening.
