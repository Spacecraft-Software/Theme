...
#
# Steelbore Green High Contrast — GNOME Terminal theme (dconf profile installer)
...
# Creates the 'Steelbore Green High Contrast' GNOME Terminal profile via dconf.
#
...
PROFILE_NAME='Steelbore Green High Contrast'
...
dconf write "${PROFILE_PATH}foreground-color" "'#00FF41'"
dconf write "${PROFILE_PATH}bold-color-same-as-fg" false
dconf write "${PROFILE_PATH}bold-color" "'#00FF41'"
dconf write "${PROFILE_PATH}bold-is-bright" true
dconf write "${PROFILE_PATH}cursor-colors-set" true
dconf write "${PROFILE_PATH}cursor-background-color" "'#00FF41'"
dconf write "${PROFILE_PATH}cursor-foreground-color" "'#0D0208'"
dconf write "${PROFILE_PATH}highlight-colors-set" true
dconf write "${PROFILE_PATH}highlight-background-color" "'#E8FFF0'"
dconf write "${PROFILE_PATH}highlight-foreground-color" "'#0D0208'"
dconf write "${PROFILE_PATH}palette" "['#0D0208', '#FF6B6B', '#00B81C', '#FFC857', '#25B85F', '#E8FFF0', '#E8FFF0', '#00FF41', '#25B85F', '#FF6B6B', '#00B81C', '#FFC857', '#25B85F', '#E8FFF0', '#E8FFF0', '#00FF41']"
