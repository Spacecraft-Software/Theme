...
#
# Steelbore Blue High Contrast — GNOME Terminal theme (dconf profile installer)
...
# Creates the 'Steelbore Blue High Contrast' GNOME Terminal profile via dconf.
#
...
PROFILE_NAME='Steelbore Blue High Contrast'
...
dconf write "${PROFILE_PATH}background-color" "'#0A1024'"
dconf write "${PROFILE_PATH}foreground-color" "'#E6F0FF'"
dconf write "${PROFILE_PATH}bold-color-same-as-fg" false
dconf write "${PROFILE_PATH}bold-color" "'#E6F0FF'"
dconf write "${PROFILE_PATH}bold-is-bright" true
dconf write "${PROFILE_PATH}cursor-colors-set" true
dconf write "${PROFILE_PATH}cursor-background-color" "'#E6F0FF'"
dconf write "${PROFILE_PATH}cursor-foreground-color" "'#0A1024'"
dconf write "${PROFILE_PATH}highlight-colors-set" true
dconf write "${PROFILE_PATH}highlight-background-color" "'#79B4FF'"
dconf write "${PROFILE_PATH}highlight-foreground-color" "'#0A1024'"
dconf write "${PROFILE_PATH}palette" "['#0A1024', '#FF8F8F', '#28C76F', '#FFC857', '#66A3FF', '#79B4FF', '#FF8A4B', '#E6F0FF', '#66A3FF', '#FF8F8F', '#28C76F', '#FFC857', '#66A3FF', '#79B4FF', '#FF8A4B', '#E6F0FF']"
